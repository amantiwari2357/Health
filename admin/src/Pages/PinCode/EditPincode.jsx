...
const EditPincode = () => {
    ...
    const fetchPincodeDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/single-pincode/${id}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:8000/api/update-pincode/${id}`,
        pincodeData
      );
  ...
  return (
    <>
      ...
      <div className="bread">
        ...
        <div className="links">
          <Link to="/all-pincodes" className="add-new">
        ...
        <form className="row g-3" onSubmit={handleSubmit}>
          ...
          <div className="col-md-6">
            <label htmlFor="shippingCharge" className="form-label">
              Shipping Charge<sup className="text-danger">*</sup>{" "}
            </label>
            <input
              type="number"
              name="shippingCharge"
