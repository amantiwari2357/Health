    ...
    try {
      ...
      if (result.isConfirmed) {
        const response = await axios.delete(
          `https://health-4-xkdr.onrender.com/api/delete-product/${productId}`
        );
  ...
  const handleStatusChange = async (productId, field, currentValue) => {
    try {
      const response = await axios.put(
        `https://health-4-xkdr.onrender.com/api/update-product/${productId}`,
        {
      ...
      toast.success(
        `${
          field === "productStatus" ? "Product status" : "Bestseller status"
  ...
  return (
    <>
      ...
      <div className="filteration">
        <div className="selects">{/* Additional filters can go here */}</div>
          ...
          <tbody>
            {data.reverse().map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  <Link to={`/edit-product/${item._id}`} className="bt edit">
